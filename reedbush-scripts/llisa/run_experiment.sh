...
do
  ...
  srun -p $1 --gres gpu:1 -t 24:00:00 -J $JOB_NAME --cpus-per-task=4 --mem-per-cpu=10240MB singularity exec --nv $SINGULARITY_IMG bin/train.sh $CONF --save_dir $SAVEDIR/run-$RUN_COUNT --num_gpus 1 $ADDITIONAL_PARAMETERS --neptune_job_name $JOB_NAME  &> $SAVEDIR/run-$RUN_COUNT/train.log &
  sleep 10
done